...
const ProductModal = ({ isOpen, onClose, product, onSave }: ProductModalProps) => {
    ...
    const loadProductData = async () => {
        ...
        setFormData({
            ...
            price_per_kg: (priceData as { price_per_kg: string } | null)?.price_per_kg || ''
        });
    };
    ...
    const resetForm = () => {
        setFormData({
            ...
            base_unit: 'kg', pricing_type: 'daily', is_active: true, price_per_kg: ''
        });
    };
    ...
    const handleChange = (
        ...
        if (e.target instanceof HTMLInputElement && e.target.type === 'checkbox') {
            setFormData(prev => ({ ...prev, [name]: e.target instanceof HTMLInputElement ? e.target.checked : false }));
        } else {
            ...
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input
                            type="number" name="price_per_kg" value={formData.price_per_kg}
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">{formData.is_active ? 'Visible to customers' : 'Hidden'}</p>
                        </div>
                ...
                <div className="p-6 border-t flex gap-3 justify-end bg-gray-50">
                    <button onClick={onClose} className="px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-100" disabled={loading}>
                        Cancel
                    </button>
                    <button onClick={handleSubmit} disabled={loading}
                        ...
                        {loading ? (
                            ...
                            <><Save className="w-5 h-5" />{product ? 'Update' : 'Create'}</>
                        )}
                    </button>
                </div>
            </div>
...
const ProductsManagement = () => {
    ...
    const fetchProducts = async () => {
        const { data, error } = await supabase
            .from('products')
            .select(`
                id, name, description, image_url, category, is_active,
                pricing_type, base_unit, category_id,
        ...
        const formattedProducts: Product[] = (data ?? []).map(p => {
            const catRel = Array.isArray(p.product_categories) ? p.product_categories[0] : p.product_categories;

            const prices = (p.daily_prices as { price_per_kg: number; effective_date: string }[] | null) ?? [];
            ...
            return {
                ...
                category:     (catRel as { name: string } | null)?.name || p.category || 'Uncategorized',
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Total',      value: products.length,      icon: Package,     bg: 'bg-blue-100',   ic: 'text-blue-600'   },
                    { label: 'Active',     value: activeCount,           icon: Eye,         bg: 'bg-green-100',  ic: 'text-green-600'  },
                    { label: 'Inactive',   value: inactiveCount,         icon: EyeOff,      bg: 'bg-red-100',    ic: 'text-red-600'    },
                    { label: 'Categories', value: categories.length - 1, icon: DollarSign,  bg: 'bg-yellow-100', ic: 'text-yellow-600' },
